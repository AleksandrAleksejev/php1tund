...
for($i=0;$i<count($colorsHTML);$i++){
    ...
    echo"<div style='background-color: $colorsHTML[$i]; width: 50%'>$colorsHTML[$i]</div>";
}
?>
</div>
<div id="yl2">
...
echo"</ol>";
    echo"<h2> Leia autode arv </h2>";
    echo"Autode arv: " .count($Auto). " tk";
    echo "br";
...
if (count($Auto)== count($Vin)){
    ...
    echo "Toyotade Arv:" . $toyotaArv;
    echo "<br>";
    echo"Audite arv: " . $audiArv;
    echo "<h2> VIN koodid,  mille märkide arv on väiksem kui 17</h2>";
    for($l=0;$l<count($Vin);$l++){
        if (strlen($Vin[$l])<17){
            echo $Auto[$l] . " - ". $Vin[$l] . "<br>";
        }
    }
    ?>
    <div id="yl3">
    ...
    Kuva img kaustast kõik pildid width=100px.
        ...
        while($rida=readdir($asukoht)){
            if($rida!="." && $rida!=".."){
                array_push($Pilt,$rida);
            }
        }
        echo "<br>";
        for($m=0;$m<count($Pilt);$m++){
            echo "<img src='content/img/$Pilt[$m]'width=100px' alt='pilt'>";
